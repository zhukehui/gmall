...
public class SkuBoundsServiceImpl extends ServiceImpl<SkuBoundsDao, SkuBoundsEntity> implements SkuBoundsService {
    ...
    public List<ItemSaleVO> queryItemSaleVOs(Long skuId) {
        ...
        //查询打折信息
        List<SkuLadderEntity> skuLadderEntities = this.ladderDao.selectList(new QueryWrapper<SkuLadderEntity>().eq("sku_id", skuId));
        if (!CollectionUtils.isEmpty(skuLadderEntities)){
            ...
            saleVO.setDesc("满" + fullCount + "件,打" + reducePrice.divide(new BigDecimal(10)).floatValue() + "折");
